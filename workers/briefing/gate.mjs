...
export function buildEdition({ runId, editionNumber, clusters, reviewsByCluster, operatorDecisions = [] }) {
  if (!Number.isInteger(editionNumber) || editionNumber < 1) throw new Error('editionNumber must be a positive integer');
  ...
  const edition = { edition: editionNumber, published_at: new Date().toISOString(), run_id: runId, stories, pipeline: { status: 'reviewed', clusters: clusters.length, approved: stories.length } };
