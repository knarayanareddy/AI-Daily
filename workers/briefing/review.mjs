...
export function validateReview(review, items) {
  ...
  if (!Array.isArray(review.evidence_urls) || review.evidence_urls.some(url => !/^https:\/\//.test(url) || !items.some(item => item.url === url))) throw new Error('review evidence URL must be HTTPS and belong to the candidate set');
  ...
  const claims = review.claim_checks.map(check => {
    ...
    if (check.evidence_urls.some(url => !/^https:\/\//.test(url) || !items.some(item => item.url === url))) throw new Error('claim evidence URL must be HTTPS and belong to the candidate set');
