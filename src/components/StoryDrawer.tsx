...
export function StoryDrawer({ story, onClose }: Props) {
  ...
  return <>
    ...
    <aside className="story-drawer open" role="dialog" aria-modal="true" aria-labelledby="drawerTitle">
      ...
      <p className="eyebrow">EVIDENCE DESK · {story.presentation?.treatment?.replaceAll('_', ' ') || 'dispatch'}</p>
      {story.image_url && <img className="drawer-image" src={assetUrl(story.image_url)} alt="" />}
      <h2 id="drawerTitle">{story.title}</h2>
      <p className="drawer-why">{story.dek}</p>
      {story.discussion_prompt && <div className="drawer-question"><p className="drawer-label">A QUESTION TO CARRY</p><p>{story.discussion_prompt}</p></div>}
