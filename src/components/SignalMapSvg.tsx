...
type Props = { stories: Story[]; selected: Story | null; onSelect: (story: Story) => void; onOpen3d?: () => void };
...
export function SignalMapSvg({ stories, selected, onSelect, onOpen3d }: Props) {
  ...
  return <section ref={mapRef} className="signal-map" aria-labelledby="signalMapTitle">
    ...
    <div className="map-controls" aria-label="Signal Field controls"><div className="relationship-filters" aria-label="Filter relationships">{relationFilters.map(filter => <button type="button" key={filter} className={relationshipFilter === filter ? 'map-control active' : 'map-control'} aria-pressed={relationshipFilter === filter} onClick={() => setRelationshipFilter(filter)}>{filter === 'all' ? 'All threads' : relationLabels[filter]}</button>)}</div><div className="field-actions"><button type="button" className={guided ? 'map-control guided active' : 'map-control guided'} aria-pressed={guided} onClick={() => { setGuided(value => !value); setGuidedIndex(0); }}>Follow the disturbance ↗</button>{onOpen3d && <button type="button" className="map-control signal-3d-toggle" onClick={onOpen3d}>Open Signal Field · 3D</button>}</div></div>
    ...
    <div className="map-legend" role="group" aria-label="Relationship legend"><span><i className="legend-line same_event" />Same event</span><span><i className="legend-line corroborates" />Corroborates</span><span><i className="legend-line contradicts" />Contradicts</span><span><i className="legend-line consequence_of" />Consequence</span></div>
    ...
    {visibleEdges.length > 0 && <div className="map-relationships" role="group" aria-label="Accessible relationship explanations"><p className="signal-label">WHY THESE SIGNALS CONNECT</p>{visibleEdges.map((edge, index) => { const from=stories.find(story => idFor(story)===edge.from_story_id); const to=stories.find(story => idFor(story)===edge.to_story_id); if (!from || !to) return null; return <button type="button" key={`${edge.from_story_id}-${edge.to_story_id}-${index}`} className="relationship-explanation" onClick={() => { setExplainedEdge(edge); onSelect(to); }}><span>{from.title} → {to.title}</span><strong>{relationLabels[edge.kind]}</strong><small>{edge.reason}</small></button>; })}</div>}
